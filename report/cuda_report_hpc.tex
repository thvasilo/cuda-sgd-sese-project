...
of features ($N_f$). In terms of runtime scaling we observe a linear increase as we
increase the features. This behavior can be explained from the fact that the number of
features mostly affect the vector operations (dot product)
that are done in the gradient calculation, which are done sequentially within each kernel.
